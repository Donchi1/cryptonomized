...
import { FaCheck, FaExclamation, FaExclamationCircle, FaPlus } from 'react-icons/fa';
...
const LatestDeposits: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setDeposits([...sampleData, ...sampleData]);
  }, []);
  ...
  return (
    <div className="bg-transparent dark:bg-gray-800 rounded-lg shadow-md overflow-hidden  w-full ">
      <div className="bg-green-100 dark:bg-green-900 p-4 border-b border-white">
        <h3 className="text-2xl font-semibold text-green-500  flex items-center">
          <FaPlus className="mr-2 text-green-600 dark:text-green-400" />
          Latest Deposits
        </h3>
      </div>

      <div className="relative h-[100px] overflow-hidden border border-white rounded-b-lg" style={{ height: '60vh', width: '100%', overflowX: 'auto' }}>
        <div 
          className="h-full overflow-y-hidden overflow-x-auto w-full"
        ...
        >
          <table className="w-full border-collapse" ref={tableRef}>
            ...
            <tbody className="transform transition-transform duration-300">
